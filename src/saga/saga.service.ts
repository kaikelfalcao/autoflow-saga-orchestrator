...
export class SagaService {
  ...
  async handleStockReserved(reply: StockReservedReply): Promise<void> {
    const saga = await this.sagaRepo.findOne({
      where: { sagaId: reply.sagaId },
    });
    if (!saga) {
      this.logger.warn(
        `Saga ${reply.sagaId} not found for stock-reserved reply`,
      );
      return;
    }
    if (saga.status === SagaStatus.RESERVED) return;
    const startedAt = new Date(saga.createdAt).getTime();
  ...
  async handleStockInsufficient(reply: StockInsufficientReply): Promise<void> {
    ...
    if (saga.status === SagaStatus.RESERVATION_FAILED) {
      this.logger.warn(
        `Saga ${reply.sagaId} already RESERVATION_FAILED; skipping`,
      );
      return;
    }
    const reason = `Stock insufficient: ${reply.failures
      .map(
        (f) =>
          `partId=${f.partId} requested=${f.requested} available=${f.available}`,
      )
      .join("; ")}`;
    ...
    try {
      ...
      this.logger.log(
        `Saga ${reply.sagaId} → RESERVATION_FAILED; order cancelled`,
      );
    } catch (err) {
  ...
  async handleStockConsumed(reply: StockConsumedReply): Promise<void> {
    ...
    if (!saga) {
      this.logger.warn(
        `Saga ${reply.sagaId} not found for stock-consumed reply`,
      );
      return;
    }
  ...
  async handleOrderCancelled(orderId: string): Promise<void> {
    ...
    if (!saga) {
      this.logger.log(
        `No active saga for order ${orderId}; nothing to release`,
      );
      return;
    }
    if (saga.status !== SagaStatus.RESERVED || !saga.reservationId) {
  ...
  async handleReservationReleased(
    reply: ReservationReleasedReply,
  ): Promise<void> {
    const saga = await this.sagaRepo.findOne({
      where: { sagaId: reply.sagaId },
    });
